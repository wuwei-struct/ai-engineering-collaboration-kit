...
后续由规划型 AI、项目负责人、CLI 或插件把你的任务意图转换成正式 CommandPack。

## 6. 为什么建议用更强模型生成任务合同？

因为生成 CommandPack 不是简单改写文字，而是要判断：
...
普通用户不需要一次学会编程。但应该逐步理解高风险工程概念。遇到权限、计费、schema、生产配置、密钥、用户数据时，应知道这是高风险。建议阅读：
